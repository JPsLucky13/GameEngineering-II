...
function BuildAssets( i_path_assetsToBuild )
	...
	do
		...
		local shouldTargetLicenseBeCopied
		...
		for i, sourceLicense in ipairs( sourceLicenses ) do
			...
			local doesLicenseExist = DoesFileExist(targetPath)
			if doesLicenseExist then
				local lastWriteTime_licenseSource = GetLastWriteTime( sourceLicense )
				local lastWriteTime_licenseTarget = GetLastWriteTime( targetPath )
				shouldTargetLicenseBeCopied = lastWriteTime_licenseSource > lastWriteTime_licenseTarget
			else
				shouldTargetLicenseBeCopied = true
			end
				
			if shouldTargetLicenseBeCopied then
	...
	do
		-- Decide if the target needs to be built
		local shouldTargetBeCopied
		do
			-- The simplest reason a target should be built is if it doesn't exist
			local doesTargetExist = DoesFileExist(GameInstallDir.."settings.ini")
			if doesTargetExist then
				-- Even if the target exists it may be out-of-date.
				-- If the source has been modified more recently than the target
				-- then the target should be re-built.
				local lastWriteTime_source = GetLastWriteTime(OutputDir.."settings.ini")
				local lastWriteTime_target = GetLastWriteTime(GameInstallDir.."settings.ini")
				shouldTargetBeCopied = lastWriteTime_source > lastWriteTime_target
			else
				shouldTargetBeCopied = true;
			end

			if shouldTargetBeCopied then
